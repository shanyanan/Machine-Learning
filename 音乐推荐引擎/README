推荐引擎可能是最为流行和出名的机器学习应用了。虽然我们现在知道机器学习领域其实还有很多子方向，推荐引擎只是其中之一，
...
2、有效果
其实对于我们之前讨论的其他机器学习算法而言，这也是正确的。不过对于推荐产品的形态而言，它的负向影响更加有限。
...
这里我们使用的数据集被称为"百万歌曲数据库"(The million song dataset)。我们将基于这个数据集，为用户推荐他们可能感兴趣的歌曲。
在这个案例中，我们会实现几种不同复杂度的推荐引擎，它们都是目前工业界使用得最为广泛的推荐技术。
